...
rmf_plot.rmf_2d_array <- function(array,
                                       dis,
                                       bas = NULL,
                                       mask = rmfi_ifelse0(is.null(bas),array*0+1,{warning('Using first ibound layer as mask.', call. = FALSE);bas$ibound[,,1]}),
  ...
  if(plot3d) {
    ...
    if(type=='fill') {  
      ...
      if(!is.null(crs)) {
        xy <- RMODFLOW:::rmfi_convert_coordinates(xy,from=CRS(prj$projection),to=crs)
      }
